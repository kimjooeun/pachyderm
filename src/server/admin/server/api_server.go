...
func (a *apiServer) Extract(request *admin.ExtractRequest, extractServer admin.API_ExtractServer) (retErr error) {
	...
	if !request.NoAuth {
		if _, err := pachClient.GetConfiguration(pachClient.Ctx(), &auth.GetConfigurationRequest{}); err == nil {
			...
			if err := writeOp(&admin.Op{Op1_11: &admin.Op1_11{CheckAuthToken: &admin.CheckAuthToken{TokenHash: authTokenHash}}}); err != nil {
				return err
			}
	...
	if !request.NoEnterprise {
		...
		if state.State == enterprise.State_ACTIVE {
			if err := writeOp(&admin.Op{Op1_11: &admin.Op1_11{ActivateEnterprise: &enterprise.ActivateRequest{ActivationCode: state.ActivationCode}}}); err != nil {
				return err
			}
		} else {
	...
	if !request.NoAuth {
		...
		if err := writeOp(&admin.Op{Op1_11: &admin.Op1_11{ActivateAuth: &auth.ActivateRequest{RootTokenHash: authTokenHash}}}); err != nil {
			return err
		}
		...
		for _, t := range robotTokens.Tokens {
			if err := writeOp(&admin.Op{Op1_11: &admin.Op1_11{RestoreAuthToken: &auth.RestoreAuthTokenRequest{Token: t}}}); err != nil {
		...
		for principal, roles := range admins.Bindings {
			if err := writeOp(&admin.Op{Op1_11: &admin.Op1_11{SetClusterRoleBinding: &auth.ModifyClusterRoleBindingRequest{
				Principal: principal,
				Roles:     roles,
			}}}); err != nil {
				return err
		...
		if err := writeOp(&admin.Op{Op1_11: &admin.Op1_11{SetAuthConfig: &auth.SetConfigurationRequest{Configuration: config.Configuration}}}); err != nil {
			return err
		}
		...
		if !request.NoRepos {
			for _, ri := range ris {
				...
				if err := writeOp(&admin.Op{Op1_11: &admin.Op1_11{SetAcl: &auth.SetACLRequest{
					Repo:    ri.Repo.Name,
					Entries: acl.Entries,
				}}}); err != nil {
					return err
